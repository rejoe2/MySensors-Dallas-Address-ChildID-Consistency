...
uint8_t DS_Start_ID = 7; //First Child-ID to be used by Dallas Bus; set this to be higher than other Child-ID's who need EEPROM storage to avoid conflicts
...
void presentation() {
  ...
  for (int i = 0; i < numSensors && i < MAX_ATTACHED_DS18B20; i++) {
    ...
    //    string_print = stringAddr(tempDeviceAddress);
    present(ts_spot[i], S_TEMP); //, ((char*) (tempDeviceAddress, 8))); //seem that this is not working as intended
#ifdef SEND_ID
